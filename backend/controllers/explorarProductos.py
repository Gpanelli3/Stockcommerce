...
def productos():
    try:
        ...
        ).outerjoin(Categoria, Stock.categoria == Categoria.id_categoria
        ).outerjoin(Proveedor, Stock.id_proveedor == Proveedor.id_proveedor
        ).all()
        ...
        listar_productos = [
            {
                ...
                "supplier": empresa if empresa is not None else "Sin proveedor",
                "category": categoria if categoria is not None else "Sin categoría"
            }
